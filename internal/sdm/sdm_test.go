...
func TestMain(m *testing.M) {
	...
	case cmdReadySuccessBehavior.String():
		executeCommand([]string{"ready"}, `{"account":"mariano.zunino@stuzo.com","listener_running":true,"state_loaded":true,"is_linked":true}`, 0)
	...
	case cmdLoginSuccessBehavior.String():
		executeCommand([]string{"login", "--email", "mariano.zunino@stuzo.com"}, `logged in`, 0)
	case cmdLoginErrorNoAccountBehavior.String():
		executeCommand([]string{"login", "--email", "mariano.zunino@stuzo.com"}, `This email doesn't have a strongDM account.`, 1)
	case cmdLoginErrorUnknownBehavior.String():
		executeCommand([]string{"login", "--email", "mariano.zunino@stuzo.com"}, `cannot ask for password`, 1)
	case cmdLoginInvalidCredentialsBehavior.String():
		executeCommand([]string{"login", "--email", "mariano.zunino@stuzo.com"}, `access denied\n`, 1)
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: SdmReady{
				...
				Account:         strPtr("mariano.zunino@stuzo.com"),
			},
			expectedErr: false,
		},
		{
